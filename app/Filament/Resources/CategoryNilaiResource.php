...
class CategoryNilaiResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Category Nilai'; // ngerubah title navigation dan label 

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Card::make() // untuk membuat form berbentuk card 
                    ->schema([
                        TextInput::make('name') // Untuk menampilkan data name
                            ->live()
                            ->afterStateUpdated(fn (Set $set, ?string $state) => $set('slug', \Str::slug($state))),  // berfungsi untuk membuat slug
                        TextInput::make('slug'),
                    ])
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                 TextColumn::make('name'), // menampilkan data didalam name
